...
enum
{
    RUN_MUSIC_TRACK_FLAG,
    PANIC_MUSIC_TRACK_FLAG,
    TRACK3_MUSIC_TRACK_FLAG,
};
...
- (instancetype) init
{
    if (self = [super init])
    {
        ...
        touchBeganInMusic = false;
        touchEndedInMusic = false;
    }
    return self;
}

//<uhhhh :[ long story, raname better to innerInitilization
- (void) create
{
    
    state = MENU_STATE;
    scoreState = LOCAL_SCORE_MODE;
    
    
    {//< init create title
        title = FlxG.iPad ? [FlxSprite spriteWithGraphic:ImgTitleIPad] : [FlxSprite spriteWithGraphic:ImgTitle];
        title.y = -FlxG.height;
    ...
    {//<init about button
        about = [[[FlxButton alloc] initWithX:8
                                     ...
                                     callback:[FlashFunction functionWithTarget:self
                                                                         action:@selector(handleAboutButtonPressed)]] autorelease];
    ...
    {//<init play button
        play = [[[FlxButton alloc] initWithX:FlxG.width-128
                                           y:FlxG.height-36
                                    callback:[FlashFunction functionWithTarget:self
                                                                        action:@selector(handlePlayButtonPressed)]] autorelease];
    ...
    //<init back button
    {
        back = [[[FlxButton alloc] initWithX:8+FlxG.width
                                           y:FlxG.height-36-4
                                    callback:[FlashFunction functionWithTarget:self
                                                                        action:@selector(handleBackButtonPressed)]] autorelease];
    ...
    {//<init bar
        bar = FlxG.iPad ? [FlxSprite spriteWithGraphic:ImgBarIPad] : [FlxSprite spriteWithGraphic:ImgBar];
        bar.x = 0;
        bar.y = -bar.height;
        [self add:bar];
    }
    ...
    {//<init about text
        aboutTitle = [FlxText textWithWidth:FlxG.width
                                       text:NSLocalizedString(@"About CNBCanabaltGame", @"About CNBCanabaltGame")
                                       font:nil
                                       size:16.0];
        ...
        aboutText = [FlxText textWithWidth:FlxG.width-textIndent
                                      text:NSLocalizedString(@"CNBCanabaltGame began as a 5-day experimental game for the Kyles’ Experimental Gameplay Project. Adam Atomic spent two weekends designing the gameplay, writing the code, drawing the artwork, and recording and designing the sound effects.  Danny Baranowsky wrote the music in one night and Adam released it into the wild.  It was slightly more popular than expected.\n\nThe original Flash game is available for free online at http://www.adamatomic.com/canabalt/\n\nEric ported CNBCanabaltGame to the iPhone the following week.  CNBCanabaltGame was developed and published independently for the iPhone by Eric and Adam Atomic under their Semi Secret imprint.",
                                                             @"CNBCanabaltGame began as a 5-day experimental game for the Kyles’ Experimental Gameplay Project. Adam Atomic spent two weekends designing the gameplay, writing the code, drawing the artwork, and recording and designing the sound effects.  Danny Baranowsky wrote the music in one night and Adam released it into the wild.  It was slightly more popular than expected.\n\nThe original Flash game is available for free online at http://www.adamatomic.com/canabalt/\n\nEric ported CNBCanabaltGame to the iPhone the following week.  CNBCanabaltGame was developed and published independently for the iPhone by Eric and Adam Atomic under their Semi Secret imprint.")
                                      font:nil
                                      size:8.0];
    ...
    {//<init thanks text
        thanksText = [FlxText textWithWidth:FlxG.width-8
                                       text:NSLocalizedString(@"Adam would like to thank some people who contributed to the success of CNBCanabaltGame:", @"Adam would like to thank some people who contributed to the success of CNBCanabaltGame:")
                                       font:nil
                                       size:8.0];
    ...
    if (musicSelection == nil || [musicSelection intValue] == RUN_MUSIC_TRACK_FLAG)
    {
        [FlxG playMusic:RUN_TITLE_MUSIC];
        musicTitle = RUN_TITLE;
    }
    else if ([musicSelection intValue] == PANIC_MUSIC_TRACK_FLAG)
    {
...
- (void) update
{
    ...
    //<so why I have states?
    //<why I need such state check?
    if (nowPlaying)
    {
    ...
    if (switchMusic)
    {
        ...
        if (musicSelection == nil || [musicSelection intValue] == RUN_MUSIC_TRACK_FLAG)
        {
            nowPlayingSelection = RUN_MUSIC_TRACK_FLAG;
        }
        //playingRun = YES;
        ...
        if (nowPlayingSelection == RUN_MUSIC_TRACK_FLAG)
        {
            [FlxG playMusic:PANIC_TITLE_MUSIC];
            musicTitle = PANIC_TITLE;
            [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithInt:PANIC_MUSIC_TRACK_FLAG]
                                                      forKey:MUSIC_SELECTION];
            [[NSUserDefaults standardUserDefaults] synchronize];
            
        } else if (nowPlayingSelection == PANIC_MUSIC_TRACK_FLAG)
        {
            [FlxG playMusic:TRACK3_TITLE_MUSIC];
            musicTitle = TRACK3_TITLE;
            [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithInt:TRACK3_MUSIC_TRACK_FLAG]
                                                      forKey:MUSIC_SELECTION];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
        else
        {
            [FlxG playMusic:RUN_TITLE_MUSIC];
            musicTitle = RUN_TITLE;
            [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithInt:RUN_MUSIC_TRACK_FLAG]
                                                      forKey:MUSIC_SELECTION];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
    ...
    if (!play.visible && title.velocity.y == 0 && state == MENU_STATE)
    {
        play.visible = YES;
        about.visible = YES;
...
- (void) handlePlayButtonPressed
{
    ...
    FlxG.state = [[[CNBPlayState alloc] init] autorelease];
